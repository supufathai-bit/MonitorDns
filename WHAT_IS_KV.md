# 📦 KV คืออะไร?

## 🎯 KV = Key-Value Store

**KV** คือ **Key-Value database** ของ Cloudflare สำหรับเก็บข้อมูลแบบง่ายๆ

---

## 💡 อธิบายง่ายๆ

**KV** เหมือน **ตู้เก็บของ** ที่มี:

- **Key** (กุญแจ) = ชื่อ/ที่อยู่ของข้อมูล
- **Value** (ของ) = ข้อมูลที่เก็บ

**ตัวอย่าง:**

```
Key: "latest:ufathai.win:AIS"
Value: {
  "hostname": "ufathai.win",
  "isp_name": "AIS",
  "status": "BLOCKED",
  "timestamp": 1703846400000
}
```

---

## 🔍 เปรียบเทียบ

### KV vs Database ปกติ

| KV | Database ปกติ (SQL) |
|----|---------------------|
| เก็บแบบ key-value | เก็บแบบ table/row |
| ง่าย เร็ว | ซับซ้อนกว่า |
| ไม่มี query ซับซ้อน | มี query ซับซ้อนได้ |
| เหมาะกับข้อมูลง่ายๆ | เหมาะกับข้อมูลซับซ้อน |

---

## 🎯 ใช้ KV ยังไง?

### ในโปรเจคเรา

**เก็บข้อมูลจาก Android app:**

```typescript
// เก็บผลลัพธ์
await env.SENTINEL_DATA.put(
  "latest:ufathai.win:AIS",  // Key
  JSON.stringify({          // Value
    hostname: "ufathai.win",
    isp: "AIS",
    status: "BLOCKED"
  })
);

// ดึงข้อมูล
const data = await env.SENTINEL_DATA.get("latest:ufathai.win:AIS");
```

---

## 📊 ข้อมูลที่เก็บใน KV

### 1. Latest Results

- Key: `latest:{hostname}:{isp}`
- Value: ผลลัพธ์ล่าสุดของแต่ละ domain+ISP

### 2. Sync History

- Key: `sync:{device_id}:{timestamp}`
- Value: ประวัติการ sync

### 3. Device Info

- Key: `device:{device_id}`
- Value: ข้อมูล device

---

## ✅ ข้อดีของ KV

1. **เร็ว** - อ่าน/เขียนเร็วมาก
2. **ง่าย** - ไม่ต้อง setup database
3. **ฟรี** - Cloudflare ให้ฟรี (100,000 reads/day)
4. **Global** - กระจายทั่วโลก

---

## ⚠️ ข้อจำกัด

1. **ไม่มี query ซับซ้อน** - ต้องรู้ key
2. **ขนาดจำกัด** - Value สูงสุด 25MB
3. **Eventually consistent** - อาจ delay นิดหน่อย

---

## 🎯 สรุป

**KV = Key-Value Store**

- เก็บข้อมูลแบบ key-value
- ง่าย เร็ว ฟรี
- เหมาะกับข้อมูลง่ายๆ เช่น ผลลัพธ์ DNS check

**ในโปรเจคเรา:**

- ใช้เก็บผลลัพธ์จาก Android app
- ใช้เก็บ sync history
- ใช้เก็บ device info

---

## 📚 เอกสารเพิ่มเติม

- [Cloudflare KV Documentation](https://developers.cloudflare.com/workers/runtime-apis/kv/)

## 🎯 KV = Key-Value Store

**KV** คือ **Key-Value database** ของ Cloudflare สำหรับเก็บข้อมูลแบบง่ายๆ

---

## 💡 อธิบายง่ายๆ

**KV** เหมือน **ตู้เก็บของ** ที่มี:

- **Key** (กุญแจ) = ชื่อ/ที่อยู่ของข้อมูล
- **Value** (ของ) = ข้อมูลที่เก็บ

**ตัวอย่าง:**

```
Key: "latest:ufathai.win:AIS"
Value: {
  "hostname": "ufathai.win",
  "isp_name": "AIS",
  "status": "BLOCKED",
  "timestamp": 1703846400000
}
```

---

## 🔍 เปรียบเทียบ

### KV vs Database ปกติ

| KV | Database ปกติ (SQL) |
|----|---------------------|
| เก็บแบบ key-value | เก็บแบบ table/row |
| ง่าย เร็ว | ซับซ้อนกว่า |
| ไม่มี query ซับซ้อน | มี query ซับซ้อนได้ |
| เหมาะกับข้อมูลง่ายๆ | เหมาะกับข้อมูลซับซ้อน |

---

## 🎯 ใช้ KV ยังไง?

### ในโปรเจคเรา

**เก็บข้อมูลจาก Android app:**

```typescript
// เก็บผลลัพธ์
await env.SENTINEL_DATA.put(
  "latest:ufathai.win:AIS",  // Key
  JSON.stringify({          // Value
    hostname: "ufathai.win",
    isp: "AIS",
    status: "BLOCKED"
  })
);

// ดึงข้อมูล
const data = await env.SENTINEL_DATA.get("latest:ufathai.win:AIS");
```

---

## 📊 ข้อมูลที่เก็บใน KV

### 1. Latest Results

- Key: `latest:{hostname}:{isp}`
- Value: ผลลัพธ์ล่าสุดของแต่ละ domain+ISP

### 2. Sync History

- Key: `sync:{device_id}:{timestamp}`
- Value: ประวัติการ sync

### 3. Device Info

- Key: `device:{device_id}`
- Value: ข้อมูล device

---

## ✅ ข้อดีของ KV

1. **เร็ว** - อ่าน/เขียนเร็วมาก
2. **ง่าย** - ไม่ต้อง setup database
3. **ฟรี** - Cloudflare ให้ฟรี (100,000 reads/day)
4. **Global** - กระจายทั่วโลก

---

## ⚠️ ข้อจำกัด

1. **ไม่มี query ซับซ้อน** - ต้องรู้ key
2. **ขนาดจำกัด** - Value สูงสุด 25MB
3. **Eventually consistent** - อาจ delay นิดหน่อย

---

## 🎯 สรุป

**KV = Key-Value Store**

- เก็บข้อมูลแบบ key-value
- ง่าย เร็ว ฟรี
- เหมาะกับข้อมูลง่ายๆ เช่น ผลลัพธ์ DNS check

**ในโปรเจคเรา:**

- ใช้เก็บผลลัพธ์จาก Android app
- ใช้เก็บ sync history
- ใช้เก็บ device info

---

## 📚 เอกสารเพิ่มเติม

- [Cloudflare KV Documentation](https://developers.cloudflare.com/workers/runtime-apis/kv/)

## 🎯 KV = Key-Value Store

**KV** คือ **Key-Value database** ของ Cloudflare สำหรับเก็บข้อมูลแบบง่ายๆ

---

## 💡 อธิบายง่ายๆ

**KV** เหมือน **ตู้เก็บของ** ที่มี:

- **Key** (กุญแจ) = ชื่อ/ที่อยู่ของข้อมูล
- **Value** (ของ) = ข้อมูลที่เก็บ

**ตัวอย่าง:**

```
Key: "latest:ufathai.win:AIS"
Value: {
  "hostname": "ufathai.win",
  "isp_name": "AIS",
  "status": "BLOCKED",
  "timestamp": 1703846400000
}
```

---

## 🔍 เปรียบเทียบ

### KV vs Database ปกติ

| KV | Database ปกติ (SQL) |
|----|---------------------|
| เก็บแบบ key-value | เก็บแบบ table/row |
| ง่าย เร็ว | ซับซ้อนกว่า |
| ไม่มี query ซับซ้อน | มี query ซับซ้อนได้ |
| เหมาะกับข้อมูลง่ายๆ | เหมาะกับข้อมูลซับซ้อน |

---

## 🎯 ใช้ KV ยังไง?

### ในโปรเจคเรา

**เก็บข้อมูลจาก Android app:**

```typescript
// เก็บผลลัพธ์
await env.SENTINEL_DATA.put(
  "latest:ufathai.win:AIS",  // Key
  JSON.stringify({          // Value
    hostname: "ufathai.win",
    isp: "AIS",
    status: "BLOCKED"
  })
);

// ดึงข้อมูล
const data = await env.SENTINEL_DATA.get("latest:ufathai.win:AIS");
```

---

## 📊 ข้อมูลที่เก็บใน KV

### 1. Latest Results

- Key: `latest:{hostname}:{isp}`
- Value: ผลลัพธ์ล่าสุดของแต่ละ domain+ISP

### 2. Sync History

- Key: `sync:{device_id}:{timestamp}`
- Value: ประวัติการ sync

### 3. Device Info

- Key: `device:{device_id}`
- Value: ข้อมูล device

---

## ✅ ข้อดีของ KV

1. **เร็ว** - อ่าน/เขียนเร็วมาก
2. **ง่าย** - ไม่ต้อง setup database
3. **ฟรี** - Cloudflare ให้ฟรี (100,000 reads/day)
4. **Global** - กระจายทั่วโลก

---

## ⚠️ ข้อจำกัด

1. **ไม่มี query ซับซ้อน** - ต้องรู้ key
2. **ขนาดจำกัด** - Value สูงสุด 25MB
3. **Eventually consistent** - อาจ delay นิดหน่อย

---

## 🎯 สรุป

**KV = Key-Value Store**

- เก็บข้อมูลแบบ key-value
- ง่าย เร็ว ฟรี
- เหมาะกับข้อมูลง่ายๆ เช่น ผลลัพธ์ DNS check

**ในโปรเจคเรา:**

- ใช้เก็บผลลัพธ์จาก Android app
- ใช้เก็บ sync history
- ใช้เก็บ device info

---

## 📚 เอกสารเพิ่มเติม

- [Cloudflare KV Documentation](https://developers.cloudflare.com/workers/runtime-apis/kv/)
